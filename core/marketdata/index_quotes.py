...
전체 항목이 다 실패하면(예: yfinance 자체 장애) 조용히 빈 리스트를 200으로
돌려주지 않는다 — 캐시된 이전 값이 있으면 그걸로 폴백하고, 캐시조차 없으면
예외를 던져 라우트가 502로 응답하게 한다(리뷰 CRITICAL 항목 수정).
"""
...
# 캐시 만료 시 동시 요청이 yfinance를 중복 호출하지 않도록 직렬화한다
# (리뷰 Important 항목 수정: check-then-act 레이스).
...
async def get_index_quotes(
    ...
    *,
    ...
    캐시 조회~갱신 전체를 락으로 감싸 동시 요청의 중복 fetch를 막는다. 전체
    티커가 다 실패하면 마지막 캐시 값으로 폴백하고(신선하지 않아도 없는 것보다
    낫다), 캐시조차 없으면 예외를 던진다 — 호출부(core/api/routes/indices.py)가
    이를 502로 변환해, "정상인데 데이터가 없음"과 "조회 자체가 실패함"을
    구분하지 못한 채 조용히 200을 반환하는 일이 없게 한다.
    """
    global _cache
    ...
    async with _cache_lock:
        ...
        if not quotes:
            if _cache is not None:
                logger.warning("전체 시세 조회 실패 — 마지막 캐시 값으로 폴백")
                return _cache[1]
            raise RuntimeError("전체 지수·환율 조회 실패 — 캐시된 값도 없음")
