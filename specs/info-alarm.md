...
> 기술 스택: Python + Toss증권 Open API + Telegram Bot + Google Calendar API
...
> **Phase 10 범위:** TradingNews API는 유료($20/월~)이므로 Phase 10에서는 구현하지 않는다. Finnhub + Yahoo RSS 무료 조합으로 운영하고, 필요 시 Phase 11에서 연동 검토.
...
> **⚠️ Phase 10 구현 범위:** 1~~6단계만 해당 (tasks.md T057~~T068). 7단계 CCI 매매 시그널 연계는 **Phase 10 범위 외** — Phase 11에서 별도 기획 예정.

---

## 10. 파일 구조 (구현 시 참고)

> **실제 경로:** `quanteo/info/` — tasks.md Phase 10 기준. `auto_trading/` 이름은 사용하지 않는다.

```
info/
├── main.py                  # InfoSystem 진입점 (start/stop 라이프사이클)
├── scheduler.py             # APScheduler 스케줄 관리 (KST 타임존 필수)
├── news/
│   ├── rss_collector.py     # RSS 뉴스 수집 (한국경제·매일경제·이데일리)
│   ├── naver_collector.py   # 네이버금융 BeautifulSoup 스크래핑 (Phase 11 예정)
│   ├── dart_collector.py    # DART 공시 수집 (OpenDartReader)
│   └── finnhub_collector.py # 해외 뉴스 수집 (Finnhub·Yahoo RSS)
├── calendar/
│   ├── google_cal.py        # Google Calendar API (gcsa, OAuth2)
│   ├── earnings_data.py     # 실적발표일 데이터 (EarningsEvent)
│   └── macro_events.py      # 경제지표 일정 데이터 (2026 H2 범위)
├── fx/
│   ├── rate_monitor.py      # 환율 수집 및 급변 감지 (yfinance)
│   ├── rate_rule.py         # 환율-주가 상관 해석 룰 테이블
│   └── daily_report.py      # 일일 환율 마감 리포트
├── ai_filter/
│   └── claude_filter.py     # Claude Haiku 중요도 분류
└── telegram/
    └── info_notifier.py     # InfoNotifier (TelegramNotifier 래핑)
```

> **설정:** API 키·경로는 `~/quanteo/config/quanteo.yaml`의 `info:` 섹션에서 로딩 (`InfoSettings(BaseModel)` 참고).  
> **구현 태스크:** [specs/tasks.md Phase 10](tasks.md#phase-10----) 참고.

---

_마지막 업데이트: 2026-06-29 | SK하이닉스(000660) 자동매매 시스템 기획_
