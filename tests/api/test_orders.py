...
async def _insert_order(store: StateStore, status: str = "submitted", side: str = "buy") -> str:
    cid = str(uuid.uuid4())
    ...
    await store.conn.execute(
        ...
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (cid, "005930", "domestic", "vps", side, "market", 5, 0.0, status, now, now),
    )
...
async def test_orders_side_normalized_to_uppercase(store):
    ...
    대시보드의 OrderItem.side 타입("BUY" | "SELL")과 계약이 어긋나면 주문
    방향이 항상 SELL로 잘못 표시되는 회귀를 이 테스트가 방지한다 —
    order_history_sync.py가 side를 소문자로 저장하는데 API가 그대로
    흘려보내던 게 원인이었다.
    """
    await _insert_order(store, status="submitted", side="buy")
    await _insert_order(store, status="submitted", side="sell")
