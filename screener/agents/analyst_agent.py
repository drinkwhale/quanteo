...
class AnalystAgent:
    ...
    async def summarize(
        self, stock: RankedStock, disclosures: list[Disclosure]
    ) -> StockSummary:
        try:
            thesis, protips, risk_flags = await self._call_claude(stock, disclosures)
        except Exception as exc:
            logger.error("Claude 요약 생성 실패(%s): %s — 정량 폴백", stock.ticker, exc)
            return self._fallback_summary(stock)
    ...
    def _fallback_summary(self, stock: RankedStock) -> StockSummary:
        return StockSummary(
            ticker=stock.ticker,
            name=stock.name,
    ...
    async def _call_claude(
        ...
        user_msg = self._build_prompt(stock, disclosures)
